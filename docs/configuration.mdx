...
<ParamField path="downloadNode" type="Property<Boolean>">
  Whether to automatically download Node.js for running tests. When `true` (the default), the plugin downloads a Node.js distribution into `build/plugwright/node`, verifies it against the official `SHASUMS256.txt` checksums from nodejs.org, and uses it for all `node`/`npm` invocations. When `false`, the system-installed `node`/`npm` are used instead.
